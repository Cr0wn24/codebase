...
#define SPALL_BUFFER_PROFILE_END(name)
...
#ifdef SPALL_DEBUG
    if(wb->ctx != ctx) return false; // Buffer must be bound to this context (or to 0)
#endif
  ...
  SPALL_FN bool spall_buffer_init(SpallProfile *ctx, SpallBuffer *wb) {
    if(!spall_buffer_flush(0, wb)) return false;
    wb->ctx = ctx;
    return true;
  }
  ...
  SPALL_FN bool spall_buffer_abort(SpallBuffer *wb) {
    if(!wb) return false;
    wb->ctx = 0;
    if(!spall__buffer_flush(0, wb)) return false;
    return true;
  }
