...
allocate_and_push_back_glyph_array(F_DWrite_GlyphArrayChunk **first_chunk, F_DWrite_GlyphArrayChunk **last_chunk)
{
  ...
  F_DWrite_GlyphArrayChunk *glyph_array_chunk = *last_chunk;
  {
    ...
    {
      glyph_array_chunk = (F_DWrite_GlyphArrayChunk *)calloc(1, sizeof(F_DWrite_GlyphArrayChunk));
      if(*first_chunk == 0)
      {
...
allocate_and_push_back_segment_node(F_DWrite_TextToGlyphsSegmentNode **first_segment, F_DWrite_TextToGlyphsSegmentNode **last_segment)
{
  F_DWrite_TextToGlyphsSegmentNode *segment_node = (F_DWrite_TextToGlyphsSegmentNode *)calloc(1, sizeof(F_DWrite_TextToGlyphsSegmentNode));
  if(*first_segment == 0)
  {
...
fill_segment_with_glyph_array_chunks(F_DWrite_TextToGlyphsSegment *segment, F_DWrite_GlyphArrayChunk *first_chunk, F_DWrite_GlyphArrayChunk *last_chunk)
{
  ...
  F_DWrite_GlyphArrayChunk *next_chunk = 0;
  for(F_DWrite_GlyphArrayChunk *chunk = first_chunk; chunk != 0; chunk = next_chunk)
  {
    next_chunk = chunk->next;
...
f_dwrite_map_text_to_glyphs(IDWriteFontFallback1 *font_fallback, IDWriteFontCollection *font_collection, IDWriteTextAnalyzer1 *text_analyzer, const wchar_t *locale, const wchar_t *base_family, const F32 font_size, const wchar_t *text, const U32 text_length)
{
  ...
  for(U32 fallback_offset = 0; fallback_offset < text_length;)
  {
    ...
    while(fallback_ptr < fallback_opl)
    {
      ...
      U64 max_glyph_indices_count = (3 * mapped_text_length) / 2 + 16;
      U16 *glyph_indices = (U16 *)calloc(max_glyph_indices_count, sizeof(U16));
      ...
      if(is_simple)
      {
        ...
        if(segment == 0)
        {
          F_DWrite_TextToGlyphsSegmentNode *segment_node = allocate_and_push_back_segment_node(&result.first_segment, &result.last_segment);
        ...
        glyph_array->advances = (F32 *)calloc(glyph_array->count, sizeof(F32));
        glyph_array->offsets = (DWRITE_GLYPH_OFFSET *)calloc(glyph_array->count, sizeof(DWRITE_GLYPH_OFFSET));
        glyph_array->indices = (U16 *)calloc(glyph_array->count, sizeof(U16));
      ...
      else
      {
        ...
        for(TextAnalysisSinkResultChunk *chunk = analysis_sink.first_result_chunk; chunk != 0; chunk = chunk->next)
        {
          for(U64 text_analysis_sink_result_idx = 0; text_analysis_sink_result_idx < chunk->count; ++text_analysis_sink_result_idx)
          {
            ...
            U16 *cluster_map = (U16 *)calloc(analysis_result.text_length, sizeof(U16));
            ...
            for(int retry = 0;;)
            {
              ...
              if(hr == HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER) && ++retry < 8)
              {
                ...
                glyph_indices = (U16 *)realloc(glyph_indices, max_glyph_indices_count * sizeof(U16));
