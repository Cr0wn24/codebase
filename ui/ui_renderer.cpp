...
ui_draw_text(Vec2F32 pos, F_Tag tag, U32 size, String8 string, Vec4F32 color)
{
  ...
  f_glyph_run_from_tag_size_str8(scratch.arena, tag, size, string);
  F32 advance = pos.x;
  Atlas *atlas = f_atlas();
  for(F_GlyphRunNode *node = glyph_run.first; node != 0; node = node->next)
  {
    F32 xpos = advance + node->bearing.x;
    F32 ypos = pos.y - node->bearing.y + y_offset;
